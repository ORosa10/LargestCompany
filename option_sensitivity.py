...
def _unit_leg_payoff(terminal_prices: np.ndarray, *, option_type: str, position: str, strike: float, volatility: float, time_to_expiry: float, risk_free_rate: float, include_premiums: bool, contract_multiplier: float, normalized_spot: float) -> np.ndarray:
    premium = black_scholes_price(spot=normalized_spot, strike=strike, time_to_expiry=time_to_expiry, volatility=volatility, risk_free_rate=risk_free_rate, option_type=option_type)
    return option_payoff(option_type, position, strike, terminal_prices, premium=premium if include_premiums else 0.0) * float(contract_multiplier)


def calculate_boundary_quantity_sensitivity(base_payoff: np.ndarray, terminal_prices: np.ndarray, curve: pd.DataFrame, *, polymarket_side: str, volatility: float, time_to_expiry: float, risk_free_rate: float, include_premiums: bool, contract_multiplier: float, normalized_spot: float = 100.0, confidences: list[float] | None = None, quantities: list[float] | None = None) -> pd.DataFrame:
    ...
    for confidence_pct in confidences:
        ...
        for quantity in quantities:
            for structure, option_component in (("Call + Put", call_payoff + put_payoff), ("Call only", call_payoff), ("Put only", put_payoff)):
                ...
                rows.append({"Structure": structure, "Boundary confidence (%)": float(confidence_pct), "Contracts per leg": float(quantity), "Call strike": float(call_strike), "Put strike": float(put_strike), "Expected payoff": expected_payoff, "Payoff SD": payoff_sd, "EV / SD": expected_payoff / payoff_sd if payoff_sd > 0 else np.nan, "Probability of loss": float((total < 0).mean())})
...
def sensitivity_heatmap(sensitivity: pd.DataFrame, *, structure: str, metric: str) -> go.Figure:
    ...
    fig = go.Figure(go.Heatmap(z=values, x=[f"{value:.0f}%" for value in pivot.columns], y=[f"{value:.2f}" for value in pivot.index], text=text, texttemplate="%{text}", colorscale=color_scale, colorbar=dict(title=metric, thickness=10), hovertemplate="Boundary confidence: %{x}<br>Contracts per leg: %{y}<br>" + metric + ": %{text}<extra></extra>"))
...
def render_boundary_quantity_sensitivity(sensitivity: pd.DataFrame, *, polymarket_side: str) -> None:
    ...
    if terminal_prices is not None and base_payoff is not None and manual_payoff is not None and len(manual_payoff) == len(terminal_prices):
        render_live_manual_profile(terminal_prices, base_payoff, manual_payoff, polymarket_side=polymarket_side, bin_width=5.0)
