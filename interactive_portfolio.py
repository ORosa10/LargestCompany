        ...
        rows.append(
            {
                ...
                "define_by": "Strike",
                "boundary_type": (
                    "Win boundary" if str(leg["Option type"]) == "Call" else "Loss boundary"
                ),
                "confidence_pct": 80.0,
...
def render_interactive_leg_editor(
    *,
    ...
    """Render readable stacked option-leg controls with reciprocal locking."""
    ...
    for row_number, row in enumerate(rows, start=1):
        ...
        summary = (
            f"Leg {row_number} | {status} | {row['position']} "
            f"{row['ticker']} {row['option_type']} | Qty {float(row['quantity']):.3f}"
        )

        with st.expander(summary, expanded=True):
            st.markdown("**Position**")
            active = st.checkbox(
                "Include this option leg in the portfolio",
                value=bool(row["active"]),
                key=f"leg_active_{row_id}",
            )

            position_left, position_right = st.columns(2)
            ticker = position_left.selectbox(
                "Ticker",
                tickers,
                index=tickers.index(row["ticker"]) if row["ticker"] in tickers else 0,
                key=f"leg_ticker_{row_id}",
            )
            quantity = position_right.number_input(
            ...
            instrument_left, instrument_right = st.columns(2)
            option_type = instrument_left.selectbox(
                "Option type",
                OPTION_TYPES,
                index=OPTION_TYPES.index(row["option_type"]),
                key=f"leg_type_{row_id}",
            )
            position = instrument_right.selectbox(
                "Trade direction",
                POSITIONS,
            ...
            st.divider()
            st.markdown("**Strike definition**")
            mode_left, mode_right = st.columns(2)
            define_by = mode_left.selectbox(
                "Choose the editable input",
                DEFINE_MODES,
                index=DEFINE_MODES.index(row["define_by"]),
                key=f"leg_mode_{row_id}",
                help="Boundary lets the model calculate the strike. Strike calculates the implied boundary confidence.",
            )
            boundary_type = mode_right.selectbox(
                "Boundary type",
                BOUNDARY_TYPES,
            ...
            if define_by == "Boundary":
                confidence_pct = value_left.number_input(
                    "Boundary confidence (%) - editable",
                    min_value=0.1,
                    max_value=99.9,
                ...
                value_right.number_input(
                    "Calculated strike - locked",
                    value=float(strike),
                    disabled=True,
                    format="%.2f",
                ...
                strike = value_left.number_input(
                    "Strike - editable",
                    min_value=0.01,
                ...
                value_right.number_input(
                    "Calculated boundary confidence (%) - locked",
            ...
            pricing_iv = st.number_input(
                "Pricing IV (annualized decimal, e.g. 0.42 = 42%)",
                min_value=0.0001,
                max_value=5.0,
                value=initial_iv,
                step=0.01,
            ...
            st.caption(
                f"Current leg: {position} {quantity:.3f} x {ticker} {option_type} "
                f"at strike {strike:.2f}; {boundary_type.lower()} confidence {confidence_pct:.1f}%."
            )

            if st.button("Remove this leg", key=f"leg_remove_{row_id}"):
                remove_id = row_id
