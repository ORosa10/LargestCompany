...
st.set_page_config(page_title="Conditional Boundaries", layout="wide")
st.title("Conditional Boundaries")
st.caption("Phase 2: identify market-cap levels and gaps where ranking probabilities cross selected thresholds.")
...
with st.sidebar:
    ...
    st.header("Targets")
    selected_winner_targets = st.multiselect("P(#1) boundary targets", WINNER_TARGETS, default=[0.50, 0.60, 0.70, 0.80], format_func=lambda value: f"{value:.0%}")
    selected_pair_targets = st.multiselect("Pairwise boundary targets", PAIR_TARGETS, default=[0.50, 0.60, 0.70], format_func=lambda value: f"{value:.0%}")

    run_button = st.button("Run boundary analysis", type="primary", use_container_width=True)
...
st.write("Market caps are refreshed from Yahoo Finance. Edit only the Phase 1 manual assumptions: annualized IV and Polymarket YES price.")
...
if simulation_inputs is None or corr is None or baseline is None:
    st.info("Run the boundary analysis to calculate conditional probability levels.")
else:
